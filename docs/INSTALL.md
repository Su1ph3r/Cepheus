...
pip install cepheus-engine==1.1.1
# Or install from a git ref:
pip install git+https://github.com/Su1ph3r/Cepheus@v1.1.1
# Or grab the wheel from the release page:
pip install https://github.com/Su1ph3r/Cepheus/releases/download/v1.1.1/cepheus_engine-1.1.1-py3-none-any.whl
```

The PyPI distribution name is `cepheus-engine` (the bare `cepheus`
name was already taken on PyPI by an unrelated project). The
installed CLI is still `cepheus` and `import cepheus` still works
unchanged.

Optional extras:

```sh
pip install 'cepheus-engine[html]'         # HTML report generator (jinja2)
pip install 'cepheus-engine[llm]'          # LLM enrichment via litellm
pip install 'cepheus-engine[html,llm]'     # both
```
...
# Linux amd64
curl -L -o cepheus \
  https://github.com/Su1ph3r/Cepheus/releases/download/v1.1.1/cepheus-linux-amd64
chmod +x cepheus
./cepheus --version

# Linux arm64
curl -L -o cepheus \
  https://github.com/Su1ph3r/Cepheus/releases/download/v1.1.1/cepheus-linux-arm64

# macOS Apple Silicon (Intel Macs: use Rosetta — `arch -x86_64 ./cepheus ...`)
curl -L -o cepheus \
  https://github.com/Su1ph3r/Cepheus/releases/download/v1.1.1/cepheus-darwin-arm64

# Windows amd64 (PowerShell)
Invoke-WebRequest `
  -Uri https://github.com/Su1ph3r/Cepheus/releases/download/v1.1.1/cepheus-windows-amd64.exe `
  -OutFile cepheus.exe
```
...
```sh
curl -L -o SHA256SUMS \
  https://github.com/Su1ph3r/Cepheus/releases/download/v1.1.1/SHA256SUMS
...
| Tag | Use case |
|---|---|
| `ghcr.io/su1ph3r/cepheus:1.1.1` | Pin to an exact version (recommended for CI) |
...
pip download --dest cepheus-offline cepheus-engine   # or use the GitHub release wheel
```

Transfer the directory to the air-gapped host and install:

```sh
pip install --no-index --find-links cepheus-offline cepheus-engine
```
